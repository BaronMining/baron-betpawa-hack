...
async def force_train(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_chat_action("typing")
    msg = await update.message.reply_text("🧠 Training model...")

    history_list = list(round_history)
    if len(history_list) < 50:
        await msg.edit_text(f"❌ Need 50+ rounds, have {len(history_list)}")
        return

    success = predictor.train(history_list, epochs=30)
    if success:
        predictor.save()
        await msg.edit_text(f"✅ Model trained on {len(history_list)} rounds!")
    else:
        await msg.edit_text("❌ Training failed")
...
def main():
    ...
    if not BOT_TOKEN or BOT_TOKEN == "":
        ...
        # Start health server anyway so Render doesn't crash-loop
        run_health_server()
        return
    ...
    app.add_handler(CommandHandler("train", force_train))
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(periodic_data_refresh, interval=30, first=8)
